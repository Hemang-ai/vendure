...
fields, but not for list or struct fields. MySQL and MariaDB do not support this option for `text` or
`localeText` fields because those values are stored as `longtext` without an index prefix length.
